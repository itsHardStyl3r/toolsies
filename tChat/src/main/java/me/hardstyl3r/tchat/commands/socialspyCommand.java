...
public class socialspyCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!sender.hasPermission("toolsies.socialspy")) {
            sender.sendMessage(l.getStringComponent("no_permission", Placeholder.unparsed("permission", "toolsies.socialspy")));
            return true;
        }
        UUID uuid = ((Player) sender).getUniqueId();
        sender.sendMessage(l.getStringComponent("socialspy.toggle_" + messagingManager.hasSocialspyToggled(uuid)));
