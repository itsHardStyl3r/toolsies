    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        if (!sender.hasPermission("toolsies.tchat")) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                    l.getString("no_permission")).replace("<permission>", "toolsies.tchat"));
            return true;
        }
        if (args.length > 1) {
                ...
                if (args[1].equalsIgnoreCase("chat")) {
                    if (args.length == 3) {
                        if (args[2].equalsIgnoreCase("toggle")) {
                            ...
                            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', l.getString("tchat.cooldown.chat.toggle"))
                                    .replace("<status>", localeManager.translateBoolean(l, chatManager.isChatCooldownEnabled())));
                            return true;
                        } else if (args[2].equalsIgnoreCase("clear")) {
                            chatManager.resetChatCooldowns();
                            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', l.getString("tchat.cooldown.chat.clear")));
                            return true;
                        }
                    }
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', l.getString("tchat.cooldown.chat.status"))
                            .replace("<status>", localeManager.translateBoolean(l, chatManager.isChatCooldownEnabled()))
