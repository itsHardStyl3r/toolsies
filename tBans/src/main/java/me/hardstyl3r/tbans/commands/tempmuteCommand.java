...
public class tempmuteCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length > 1) {
            ...
            sender.sendMessage(l.getStringComponent("tempmute.tempmute", Placeholder.unparsed("name", target), Placeholder.unparsed("duration", localeManager.parseTimeWithTranslate(duration, l))));
            Player p = Bukkit.getPlayerExact(target);
            if (Bukkit.getPlayerExact(target) != null) {
                Locale pl = userManager.determineLocale(p);
                p.sendMessage(pl.getStringComponent("tempmute.tempmute_target", Placeholder.unparsed("admin", punishment.getAdmin()), Placeholder.unparsed("reason", (punishment.getReason() == null ? "brak" : punishment.getReason())), Placeholder.unparsed("duration", localeManager.parseTimeWithTranslate(duration, pl))));
            }
        } else {
