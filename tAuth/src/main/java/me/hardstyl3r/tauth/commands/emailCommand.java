        ...
        if (args.length == 0) {
            if (currentEmail == null) {
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                        l.getString("email.email_no_email")));
                return true;
            }
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                    l.getString("email.email")).replace("<email>", currentEmail));
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender.hasPermission("toolsies.email")) {
            ...
            if (args.length == 2 && args[0].equalsIgnoreCase("set"))
                return Collections.singletonList(localeManager.formatArgument(l.getString("common.email"), true));
        }
